...
export default function Sidebar() {
  ...
  const handleLogout = () => {
    // در آینده logout API را صدا می‌زنیم
  ...
  return (
    <aside className="w-80 bg-white border-l border-gray-200 h-screen fixed right-0 top-0 shadow-2xl overflow-hidden z-50 transition-all duration-300 flex flex-col">
      {/* لوگو و هدر */}
      <div className="relative shrink-0 bg-gradient-to-br from-white via-gray-50 to-blue-50/30 p-8 border-b border-gray-200">
        ...
        <Link href="/" className="relative flex flex-col items-center gap-4 group">
        ...
        <div className="relative mt-5 text-center">
          <h2 className="text-gray-800 font-bold text-2xl tracking-wider mb-2 bg-gradient-to-l from-seraj-primary via-seraj-secondary to-seraj-accent bg-clip-text text-transparent">
            سامانه تحلیلبات
          </h2>
          <p className="text-gray-600 text-sm font-medium tracking-wider">
            سیستم تحلیل و مدیریت هوشمند
          </p>
        </div>
      </div>
              ...
              <li key={item.href}>
                <Link
                  ...
                  className={`
                    ...
                    ${isActive 
                      ? 'bg-gradient-to-l from-seraj-primary via-seraj-secondary to-seraj-accent text-white font-bold shadow-xl shadow-seraj-primary/30 scale-[1.02]' 
                      : 'text-gray-600 hover:bg-seraj-primary/5 hover:text-seraj-primary hover:translate-x-[-8px] border border-transparent hover:border-seraj-primary/20 hover:shadow-md'
                    }
                  `}
                >
                  {/* خط کنار برای آیتم فعال - ضخیم‌تر و با گرادیانت */}
                  {isActive && (
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 w-2 h-14 bg-white rounded-l-full shadow-lg"></div>
                  )}
                  
                  <div className={`
                    shrink-0 w-14 h-14 rounded-xl flex items-center justify-center transition-all duration-300
                    ${isActive 
                      ? 'bg-white/20 backdrop-blur-sm text-white shadow-lg scale-105' 
                      : 'bg-gray-100 text-gray-600 border border-gray-200 group-hover:bg-seraj-primary/10 group-hover:text-seraj-primary group-hover:scale-110 group-hover:border-seraj-primary/30 group-hover:shadow-lg'
                    }
                  `}>
                    <Icon className={`w-7 h-7 shrink-0 transition-transform duration-300 ${isActive ? '' : 'group-hover:scale-110 group-hover:rotate-12'}`} />
                  </div>
                  
                  <span className={`text-base flex-1 whitespace-nowrap tracking-wide ${isActive ? 'font-bold' : 'font-medium'}`}>
                    {item.label}
                  </span>
                  ...
                  {isActive && (
                    <div className="flex items-center gap-1.5 mr-auto">
                      <div className="w-2.5 h-2.5 rounded-full bg-white animate-pulse shadow-lg"></div>
                  ...
                  {!isActive && (
                    <div className="mr-auto opacity-0 group-hover:opacity-100 transition-all duration-300">
                      <div className="w-2 h-2 rounded-full bg-seraj-accent shadow-lg shadow-seraj-accent/50"></div>
                    </div>
                  )}
                </Link>
              </li>
      ...
      <div className="shrink-0 w-full p-3 border-t border-gray-200 bg-gradient-to-t from-gray-50/50 to-white">
        <button 
          ...
          className="group flex items-center gap-4 px-5 py-3 rounded-2xl transition-all duration-300 w-full bg-gradient-to-r from-white-500 via-white-600 to-white-700 text-black hover:text-white  hover:from-red-600 hover:via-red-700 hover:to-red-800  hover:shadow-2xl hover:shadow-red-500/40 hover:scale-[1.03] active:scale-[0.97]"
        >
          <div className="shrink-0 w-10 h-10 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center group-hover:bg-white/30 transition-all duration-300 group-hover:scale-110 group-hover:rotate-12">
            <IconLogout className="w-7 h-7 shrink-0 transition-transform duration-300" />
          </div>
